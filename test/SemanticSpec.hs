...
main = hspec $ do
  describe "elaborateProgram" $ do
      ...
      case parseProgram src of
        ...
        Right prog ->
          elaborateProgram prog `shouldSatisfy` \case
            Right (Program _ (VarDecls [vx, vy]) _) ->
      ...
      expectParsed src $ \prog ->
        elaborateProgram prog
          `shouldFailWith1` ( \case
                                TypeMismatch {dName = "x"} -> True -- 位置は無視
                                _ -> False
                            )
    it "rejects duplicate variable names" $ do
      ...
      expectParsed src $ \prog ->
        elaborateProgram prog
          `shouldFailWith1` ( \case
                                DuplicateVar {dName = "x"} -> True -- 前/今の位置は無視
                                _ -> False
                            )
    ...
    it "rejects non-initialized const" $ do
      ...
      expectParsed src $ \prog ->
        elaborateProgram prog
          `shouldFailWith1` ( \case
                                MissingInitializer {dName = "x"} -> True
                                _ -> False
                            )
  ...
  describe "assignment semantics" $ do
    ...
    it "rejects type mismatch" $ do
      ...
      case parseProgram src of
        ...
        Right p ->
          elaborateProgram p `shouldSatisfy` \case
            Left [TypeMismatch {dName = "x"}] -> True -- 位置はワイルドカード
            _ -> False
  ...
  describe "assignment semantics (arithmetic operations)" $ do
    let src = "PROGRAM P\nVAR\nx:INT;y:BOOL;\nEND_VAR\n"
        ...
        ng =
          -- [ ( "y := 1 AND 2;",
          --     \case
          --       OpTypeMismatch {expected = BOOL, actual = INT} -> True -- actualは暗黙の型変換いれた後はUSINTになる予定
          --       _ -> False
          --   ),
          [ ( "y := TRUE + 1;",
              \case
                OpTypeMismatch {expected = INT, actual = BOOL} -> True
                _ -> False
            )
          ]
    forM_ ok $ \s ->
      it ("parses " <> T.unpack s) $
        shouldParse (T.append src s)
    forM_ ng $ \(tc, predDiag) ->
      it ("rejects " <> T.unpack tc) $
        case parseProgram (T.append src tc) of
          Left e -> expectationFailure (show e)
          Right p -> elaborateProgram p `shouldFailWith1` predDiag
  ...
  describe "NOT semantics" $ do
    ...
    it "rejects assigning NOT 2 (INT) to BOOL" $ do
      expectParsed (base <> "y := NOT 2;\n") $ \p ->
        elaborateProgram p `shouldSatisfy` \case
          Left [OpTypeMismatch {op = "NOT", expected = BOOL, actual = INT}] -> True
          _ -> False

    it "rejects assigning NOT TRUE (BOOL) to INT" $ do
      expectParsed (base <> "x := NOT TRUE;\n") $ \p ->
        elaborateProgram p `shouldSatisfy` \case
          Left [TypeMismatch {dName = "x", expected = INT, actual = BOOL}] -> True
          _ -> False
      ...
      case parseUnit src of
        ...
        Right u ->
          elaborateUnit u `shouldSatisfy` \case
            Right (Unit _ [Program _ (VarDecls [v]) _]) ->
  ...
  describe "FOR semantics" $ do
    it "accepts simple FOR over INT and forbids writing to loop var" $ do
    ...
    it "rejects non-INT init/end/by" $ do
      let initBad = "PROGRAM P\nVAR i:INT; END_VAR\nFOR i := TRUE TO 1 DO END_FOR\n"
  ...
  describe "nominal identity (ENUM)" $ do
    ...
    it "accepts equality only for same enum name" $ do
      let ok =
            "TYPE A : (X); END_TYPE\n\
            \TYPE B : (X); END_TYPE\n\
            \PROGRAM P\nVAR a: A; END_VAR\n\
  ...
  describe "ENUM explicit values (semantics integration)" $ do
    it "accepts assignment to an enum with explicit values" $ do
      let src =
            ...
            \PROGRAM P\nVAR c: Color; END_VAR\n\
            \c := Color.Green;\n"
      shouldParseUnit src -- elaborateUnit の Right を確認
  ...
  describe "MOD/XOR semantics" $ do
    ...
    it "accepts XOR on BOOL" $
      shouldParse (base <> "b := TRUE XOR FALSE;\n")
  ...
  describe "REAL semantics (minimal)" $ do
    it "allows arithmetic mixing INT and REAL; result is REAL" $ do
      ...
      shouldParse src -- elaborateProgram succeeds
  ...
  describe "STRING default init (semantics)" $ do
    it "fills empty string for STRING" $ do
      ...
      expectParsed src $ \prog ->
        elaborateProgram prog `shouldSatisfy` \case
          Right (Program _ (VarDecls [v]) _) -> varInit v == Just (ESTRING "")
          _ -> False
    ...
    it "fills empty string for STRING(80) and STRING[32]" $ do
      ...
      expectParsed s1 $ \p1 ->
        elaborateProgram p1 `shouldSatisfy` \case
          Right (Program _ (VarDecls [v]) _) -> varInit v == Just (ESTRING "")
          _ -> False
      expectParsed s2 $ \p2 ->
        elaborateProgram p2 `shouldSatisfy` \case
          Right (Program _ (VarDecls [v]) _) -> varInit v == Just (ESTRING "")
          _ -> False
  ...
  describe "Bitwise operators over bitstrings (semantics)" $ do
    ...
    it "rejects AND on INT (bitwise only for BOOL/BITSTRING)" $ do
      let src =
            ...
            \i := i AND i;\n"
      expectParsed src $ \p ->
        elaborateProgram p `shouldFailWith1` opTypeMismatch "AND"
    ...
    it "rejects NOT on INT" $ do
      let src =
            ...
            \i := NOT i;\n"
      expectParsed src $ \p ->
        elaborateProgram p `shouldFailWith1` opTypeMismatch "NOT"
    ...
    it "rejects OR with REAL" $ do
      let src =
            ...
            \w := w OR r;\n"
      expectParsed src $ \p ->
        elaborateProgram p `shouldFailWith1` opTypeMismatch "OR"
    ...
    it "rejects BOOL with WORD (no cross-kind mixing)" $ do
      let src =
            ...
            \b := b XOR w;\n"
      expectParsed src $ \p ->
        elaborateProgram p `shouldFailWith1` opTypeMismatch "XOR"
  ...
  describe "CHAR/WCHAR/WSTRING semantics" $ do
    ...
    it "rejects assigning STRING literal to WSTRING variable" $ do
      let src =
            ...
            \ws := 'abc';\n"
      expectParsedUnit src $ \u ->
        elaborateUnit u `shouldFailWith1` typeMismatch "ws"
    ...
    it "rejects assigning multi-char STRING literal to CHAR" $ do
      let src =
            ...
            \c := 'AB';\n"
      expectParsedUnit src $ \u ->
        elaborateUnit u `shouldFailWith1` typeMismatch "c"
    ...
    it "rejects assigning multi-char WSTRING literal to WCHAR" $ do
      let src =
            ...
            \wc := \"AB\";\n"
      expectParsedUnit src $ \u ->
        elaborateUnit u `shouldFailWith1` typeMismatch "wc"
    ...
    it "rejects STRING vs WSTRING comparison" $ do
      let src =
            ...
            \b := s = w;\n"
      expectParsedUnit src $ \u ->
        elaborateUnit u `shouldFailWith1` opTypeMismatch "="
  ...
  describe "Array aggregate initialization (semantics)" $ do
    ...
    it "rejects too-many initializer elements" $ do
      ...
      expectParsedUnit src $ \u ->
        elaborateUnit u `shouldFailWith1` arrayLenMismatch
  ...
  describe "Struct aggregate initialization (semantics)" $ do
    ...
    it "rejects unknown field in aggregate" $ do
      ...
      expectParsedUnit src $ \u ->
        elaborateUnit u `shouldFailWith1` unknownFieldInit
